  ...
  if (currentPage == 1) {
        ...
        if (exersSolved[id + i]) {
            if (currentPage == 1) {
              ...
              move1Btns[i].addEventListener("click", function () {
                boardInit(allBoardsDiv[i].id, false, move1Fen[i], level1OriP1[i],onMouseoverSquare)
              ...
              move1Btns[i].addEventListener("click", function () {
                boardInit(allBoardsDiv[i].id, false, move1Fen[i], level1OriP2[i],onMouseoverSquare)
                startBtns[i].style.backgroundColor = "white"
                move1Btns[i].style.backgroundColor = "grey"
              })
            } else if (currentPage == 3) {
              ...
              move1Btns[i].addEventListener("click", function () {
                boardInit(allBoardsDiv[i].id, false, move1Fen[i], level1OriP3[i],onMouseoverSquare)
                startBtns[i].style.backgroundColor = "white"
                move1Btns[i].style.backgroundColor = "grey"
              })
            }
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (currentPage == 1) {
        ...
        boardInit(allBoardsDiv[i].id, draggable, level1StartFenP1[i], level1OriP1[i],onMouseoverSquare)
      } else if (currentPage == 2) {
        draggable = (exersSolved[i + 6]) ? false : true//page2
        boardInit(allBoardsDiv[i].id, draggable, level1StartFenP2[i], level1OriP2[i],onMouseoverSquare)
      } else if (currentPage == 3) {
        draggable = (exersSolved[i + 12]) ? false : true//page3
        boardInit(allBoardsDiv[i].id, draggable, level1StartFenP3[i], level1OriP3[i],onMouseoverSquare)
      }
    }
  }
    ...
    noLockExer = nextExerToSolve()
      locks.forEach((lock, i) => {
        if (currentPage == 1) {
          ...
          if (exersSolved[i + 6]) {
            status1[i].innerHTML = "CHECKMATE"
          } else if ((i != nextExerToSolve() && checkIfPage1Solved()) || !checkIfPage1Solved()) {
            status1[i].innerHTML = "LOCKED"
            lock.style.visibility = "visible"
          }
        } else if (currentPage == 3) {
...
function boardInit(boardId, draggable, position, orientation,onMouseoverSquare) {
  let id = getNumFromString(boardId) - 1
...
function reMakeMove1(i, move, orientation) {
  ...
  move1Btns[i].addEventListener("click", function () {
    boardInit(allBoardsDiv[i].id, false, move1Fen[i], orientation,null)
    startBtns[i].style.backgroundColor = "white"
    move1Btns[i].style.backgroundColor = "grey"
    move2Btns[i].style.backgroundColor = "white"
    checkIfStartPos[i] = false
...
function reMakeMove2(i, move, orientation) {
  ...
  move2Btns[i].addEventListener("click", function () {
    boardInit(allBoardsDiv[i].id, true, move2Fen[i], orientation,null)
    startBtns[i].style.backgroundColor = "white"
    move1Btns[i].style.backgroundColor = "white"
    move2Btns[i].style.backgroundColor = "grey"
    checkIfStartPos[i] = false
